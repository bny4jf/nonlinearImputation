...
true = [1,0.5,-2, 1.2, -0.7, 0.3, 1]#, -1.2]
...
def tableCleaner(fname, true, histogram=2):
    ...
    resnew = numpy.stack([arr for arr in res1 if numpy.max(numpy.abs(arr))<12])
    ...
    if not histogram == None:
        for i in range(resnew.shape[2]):
            hist(resnew[:, histogram, i])
...
for n in [1000, 2000, 4000]:
    print('n=', n)
    res1 = tableCleaner('5ZsO124'+str(n)+'.npy', true, 3)
 #   print(res1)
    print('\n\n\n')
